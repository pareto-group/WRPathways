...
class MiniSom(object):
    ...
    def __init__(self, input_len,
        ...
        # if statement needed for topology check
        self._mask = create_hexmask(num)

        # if statement needed for topology check
        self._weights = np.ma.array(
            self._weights,
            mask=np.repeat(
                np.expand_dims(
                    self._mask, 
                    axis=2), 
                repeats=input_len, axis=2)
        )
        
        # self._activation_map = zeros((x, y))
        self._activation_map = np.ma.array(
            zeros((x, y)),
            mask=self._mask
        )
        ...
        # if statement needed for topology check
        self._xx = np.ma.array(
            self._xx,
            mask=self._mask
        )
        self._yy = np.ma.array(
            self._yy,
            mask=self._mask
        )
    ...
    def _activate(self, x):
        ...
        self._activation_map = np.ma.array(
            self._activation_distance(x, self._weights), mask=self._mask)
    ...
    def winner(self, x):
        ...
        self._activate(x)
        return unravel_index(self._activation_map.argmin(fill_value=10000000),
    ...
    def random_weights_init(self, data):
        ...
        #    it.iternext()
        # new code for masked array
        it = np.nditer(
            [self._activation_map, self._activation_map.mask],
            flags=['multi_index'],
            op_flags=[['readwrite'], ['readonly']]
        )
        while not it.finished:
            if not np.ma.is_masked(self._weights[it.multi_index]):
                rand_i = self._random_generator.randint(len(data))
                self._weights[it.multi_index] = data[rand_i]
            it.iternext()
...
def create_hexmask(num):
    
    testmask = np.zeros((xy_using_n(num), xy_using_n(num)))
    
    # Change the mask depending on the size of the SOM
    if num % 2 == 0:
        for i in range(xy_using_n(num)):
            
            testmask[i, :int(((np.abs((num - 1) - i) - 1) // 2) + 1)] = 1.
            
            if (-int(np.abs((num - 1) - i) // 2)) == 0:
                continue

            testmask[i, -int(np.abs((num - 1) - i) // 2):] = 1.
    ...
    assert np.all(np.abs(np.diff(np.sum(testmask, axis=1))) == 1.)
    assert np.sum(testmask, axis=1)[0] == xy_using_n(num) - num
    assert np.sum(testmask, axis=1)[-1] == xy_using_n(num) - num
    assert np.sum(testmask[num-1]) == 0
